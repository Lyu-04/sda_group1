...
- `scripts/regresion anlysis/tune_models.py` - Tunes hyperparameters for XGBOOST, Random Forest and Random Forest Regression.
## Requirements
- Python 3.x
### Packages
- pandas
...
3) In the file regression_tornado.py we were overestimating the expected fatalities so with the advice from google an AI we decided to calibrate the probabilities calculated by XGBOOST, helped to code lines 273-283 in `regression_tornado.py`.
4) Wrote a code to tune the hyperparamaters of our models, never done this before so `tune_models.py` mostly written by AI.
5) In `regression_tornado.py` some code was just hard to write so for the time purposes and not to get stuck, AI helped in some lines( for example taking data in chunks because dataset was too large) but nothing significant.